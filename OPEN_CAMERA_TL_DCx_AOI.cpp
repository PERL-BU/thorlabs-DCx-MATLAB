...
// mex  'OPEN_CAMERA_TL_DCx_AOI.cpp' 'uc480.lib' ;
...
{     
    ...
    if (nrhs != 1) 
    { 
       mexErrMsgTxt("1 input required to define AOI size"); 
    }
    else if (nlhs != 2) 
    { 
  ...
  // Get AOI Info
	...
	nX = pWidth;  
    nY = pWidth;	// Modify the size of AOI size
	...
	IS_RECT rectAOI;
	rectAOI.s32X = 640-pWidth/2;
	rectAOI.s32Y = 384-pWidth/2;
	rectAOI.s32Width = nX;
